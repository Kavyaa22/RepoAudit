...
class IgnoreRules:
    ...
    def from_root(cls, root: Path) -> "IgnoreRules":
        ...
        for name in (".gitignore", ".repoauditignore"):
            path = root / name
            if not path.exists():
                continue
...
def build_file_tree(files: list[FileInfo], max_lines: int = 400) -> str:
    """render an ascii tree from the file list, similar to `tree` command."""
    # collect unique directories + files (always POSIX so indent is OS-independent)
    entries: set[str] = set()
    for f in files:
