...
def test_scan_skips_generated_bundle_lines(tmp_path):
    ...
    assert all("\\" not in f.path for f in files)


def test_scan_respects_gitignore_and_repoauditignore(tmp_path):
    (tmp_path / ".gitignore").write_text("dist/\n*.log\n", encoding="utf-8")
    (tmp_path / ".repoauditignore").write_text("private.md\n", encoding="utf-8")
    (tmp_path / "dist").mkdir()
...
def test_scan_stores_nested_paths_as_posix(tmp_path):
    nested = tmp_path / "frontend" / "src" / "pages"
    nested.mkdir(parents=True)
    (nested / "home.tsx").write_text("export const Home = () => null;\n", encoding="utf-8")

    files = scan_directory(tmp_path)

    assert files[0].path == "frontend/src/pages/home.tsx"
